 ...
 <div class="findSomeOne">
  ...
  <nav class="findNavBar">
      <Row :gutter="0" class="NavRow">
          ...
          <Col :span="5" class="NavCol">
          <router-link class="navLink" to="/rank/list/news">漂泊志</router-link>
          </Col>
      </Row>
  </nav>
  <!-- /nav -->
  ...
  <div class="RankList">
    ...
    <div v-else-if = "type == 'news'">
      <rank-list-item type="dayNews" :items="news.day" title="今日漂泊志排行榜"></rank-list-item>
      <rank-list-item type="weekNews" :items="news.week" title="一周漂泊志排行榜"></rank-list-item>
      <rank-list-item type="monthNews" :items="news.month" title="本月漂泊志排行榜"></rank-list-item>
    </div>
  </div>
</div>   
</template>
